...
vector<pair<pair<int, int>, double>> generarRayosVertices(int tam, int cantRayos){
	...
	std::vector<pair<pair<int, int>, double>> resultado (4*cantRayos);

	int aux = tam -1;
	pair<int, int> SI;// = (tam -1, 0) Extremo Superior Izquierdo
	SI.first = aux;
	SI.second = 0;

	pair<int, int> SD;// = (tam -1, tam -1) Extremo Superior Derecho
	SD.first = aux;
	SD.second = aux;	

	pair<int, int> II;// = (0, 0) Extremo Inferior Izquierdo
	II.first = 0;
	II.second = 0;

	pair<int, int> ID;// = (0, tam -1) Extremo Inferior Derecho
	ID.first = 0;
	ID.second = aux;
